# Sfida 3: (Inter)Facce Pulite - Python Edition
...
Hai ereditato una libreria Python per gestire metadati nascosti nelle immagini PNG tramite steganografia LSB. La libreria permette di salvare e leggere dati JSON incorporati direttamente nei pixel, ma le sue interfacce sono... problematiche.
...
4. **Nomi poco chiari:** `proc_meta` fa troppo e non è chiaro cosa
5. **Nessun default sensato:** tutto deve essere specificato esplicitamente, anche le cose più ovvie
6. **Tipi di ritorno inconsistenti:** a seconda dell'operazione ricevi `dict`, `bool`, `PathLike`, `None` o altri tipi casuali
...
- Crea funzioni helper private (con `_` all'inizio del nome) per tenere a bada la complessità
...
- Considera di restituire oggetti o dizionari con dati strutturati invece di valori sparsi

## Come partecipare

1. Clona il repository
2. Crea un branch: `git switch -c sfida3-tuonome`
3. Modifica `challenge-3/python/steganography.py`
...
Il file `steganography.py` non è eseguibile in quanto è una libreria. Puoi realizzare un programma di test per tuo conto, ma probabilmente il modo migliore di vedere come funziona (e **se** funziona) è leggendo ed eseguendo i test.
...
La nostra libreria dipende da Pillow, una libreria open source di manipolazione immagini. Per installarla usa il comando:

```bash
pip install pillow  # serve eseguirlo solo la prima volta
```
...
python -m pytest challenge-3/python/steganography_tests.py -v
```

L'output dei test può essere piuttosto rumoroso. Puoi limitarlo specificando da riga di comando quale suite di test eseguire:

```bash
# verifica funzionamento corretto della funzione proc_meta (non regressione)
python -m pytest challenge-3/python/steganography_tests.py::TestProcMeta -v
# verifica funzionamento corretto della funzione batch_proc (non regressione)
python -m pytest challenge-3/python/steganography_tests.py::TestBatchProc -v

# test che dimostrano i problemi esistenti con la soluzione attuale
python -m pytest challenge-3/python/steganography_tests.py::TestInterfaceProblems -v
# test che suggeriscono POSSIBILI ri-implementazioni
python -m pytest challenge-3/python/steganography_tests.py::TestNewInterfaceIdeas -v
```
